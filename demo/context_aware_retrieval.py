...
Hệ thống retrieval thông minh tự động lấy chunks liên quan để bảo toàn ngữ cảnh
"""
...
class ContextAwareRetrieval:
    """
    Hệ thống retrieval thông minh với context linking
    """
    
    def __init__(self, vector_store: SimpleVectorStore, chunks_file: str = "context_aware_chunks.json"):
...
def main():
    ...
    print("🔍 Testing Context-Aware Retrieval System")
    print("=" * 60)
    
    # Load vector store
    vector_store = SimpleVectorStore()
    if not vector_store.load('legal_vectors.pkl'):
        print("❌ Cannot load vector store. Please run simple_vector_store.py first.")
        return
    
    # Initialize context-aware retrieval
    retrieval = ContextAwareRetrieval(vector_store, "context_aware_chunks.json")
